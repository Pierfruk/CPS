...
export function setSkipStarsOn() {
    state.skipStars = true;
    return this;
}
export function setSkipStarsOff() {
    state.skipStars = false;
    return this;
}
...
function startSketch() {
    ...
    if (state.skipStars) {
        limit = cdat.lines.start.length+1;
    }
    else {
       limit= cdat.stars.x.length;
    }
...
function drawStar(x, y, Vmag) {
    let scale = state.sizeScale;
    if (state.skipStars) {
        scale = scale * 3; 
    }
    const r = (7 - Vmag) * state.canvasScale / 2000 * scale + 0.5;
...
function drawLine(x1, x2, y1, y2) {
    state.ctx.beginPath();
    state.ctx.strokeStyle = '#59a3ff';
    state.ctx.lineWidth = Math.max(2, 2 * state.canvasScale / 500);
    state.ctx.shadowColor = "rgba(255, 255, 255)";
